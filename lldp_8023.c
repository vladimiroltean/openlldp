...
static u16 ieee8023_bld_preempt_status(struct ieee8023_data *bd)
{
	...
	err = ethtool_mm_get_state(bd->ifname, &mm);
	if (err) {
	...
	status |= LLDP_8023_ADD_ETH_CAPS_ADD_FRAG_SIZE(mm.add_frag_size);
...
static void ieee8023_add_eth_caps_teardown(struct ieee8023_data *bd)
{
	...
	if (bd->enabled_preemption) {
		ethtool_mm_change_pmac_enabled(bd->ifname, false);
		ethtool_mm_change_tx_enabled(bd->ifname, false, false, 10);
	}
}
...
static void ieee8023_free_data(struct ieee8023_user_data *ud)
{
	...
	if (ud) {
		while (!LIST_EMPTY(&ud->head)) {
			...
			ieee8023_add_eth_caps_teardown(bd);
...
static int ieee8023_rchange_add_eth_caps(struct ieee8023_data *bd, u8 *buf,
					 int len)
{
	bool lp_supported, lp_enabled, lp_active;
	u32 lp_add_frag_size_bytes;
	struct ethtool_mm mm;
	u8 lp_add_frag_size;
	u8 tmp[2] = {};
	u16 preempt;
	...
	lp_add_frag_size_bytes = 64 * (1 + lp_add_frag_size) - 4;
	...
	LLDPAD_INFO("%s: Link partner additional fragment size: %d octets\n",
		    bd->ifname, lp_add_frag_size_bytes);
	...
	if (!mm.supported || !lp_supported)
		goto out;

	/* In case the pMAC is not powered on, power it up now so that
	 * it responds to SMD-V frames initiated by the link partner,
	 * who may enable their verification state machine
	 */
	if (!mm.pmac_enabled) {
		err = ethtool_mm_change_pmac_enabled(bd->ifname, true);
		if (err) {
	...
	/* Make sure that we're configured to use what minimum fragment
	 * size the link partner has requested
	 */
	if (mm.add_frag_size != lp_add_frag_size_bytes) {
		err = ethtool_mm_change_add_frag_size(bd->ifname,
						      lp_add_frag_size_bytes);
		if (err) {
			LLDPAD_ERR("%s: Failed to change addFragSize: %d\n",
				   bd->ifname, err);
			goto out;
		}
	}
	 ...
	 * partner may do. Since we request verification (using a large
	 * interval, which allows for a bit of wiggle room), preemption will
	 * only become active once the verification process succeeds.
	 */
	if (!mm.tx_enabled) {
		LLDPAD_INFO("%s: initiating verification process with an SMD-V frame interval of %d ms...\n",
			    bd->ifname, mm.max_verify_time);

		err = ethtool_mm_change_tx_enabled(bd->ifname, true, true,
						   mm.max_verify_time);
		if (err) {
