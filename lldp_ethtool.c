...
static int ethtool_msg_mm_get_parse(struct nl_msg *nlm, void *arg)
{
	...
	if (tb[ETHTOOL_A_MM_SUPPORTED])
		mm->supported = nla_get_u8(tb[ETHTOOL_A_MM_SUPPORTED]);
	...
	if (tb[ETHTOOL_A_MM_ADD_FRAG_SIZE])
		mm->add_frag_size = nla_get_u32(tb[ETHTOOL_A_MM_ADD_FRAG_SIZE]);
...
int ethtool_mm_change_add_frag_size(const char *ifname, u32 add_frag_size)
{
	...
	nla_put_u32(nlm, ETHTOOL_A_MM_ADD_FRAG_SIZE, add_frag_size);
